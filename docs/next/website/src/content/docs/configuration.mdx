...
Use process passthrough when a direct chord should belong to a pane app only while an exact process is in the foreground:
...
When the focused process name or `argv[0]` basename matches an entry, Herdr sends the original key to that pane. Otherwise, Herdr runs its normal client-local binding for the chord. Passthrough entries accept direct modified chords; prefix bindings are rejected.
