...
herdr agent restart [--prompt TEXT]
herdr agent exit
...
`agent restart` and `agent exit` act on the current pane from `HERDR_PANE_ID`. Restart requires an exact recorded native session, resumes that session in the same pane, and sends an optional prompt after the resumed agent becomes idle. Exit replaces a recognized unmanaged agent with the pane shell. Managed agents must use their lifecycle owner instead.
