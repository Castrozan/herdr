...
use super::widgets::contrast_fg_on;
...
const MAX_DERIVED_TAB_LABEL_WIDTH: usize = 22;

type Terminals = HashMap<TerminalId, TerminalState>;
...
fn tab_width(ws: &crate::workspace::Workspace, terminals: &Terminals, tab_idx: usize) -> u16 {
...
fn tab_chrome_label(
    ...
    let name = ws
        .tab_display_name_from(tab_idx, terminals)
        .unwrap_or_else(|| (tab_idx + 1).to_string());
    let name = if tab.is_some_and(|tab| tab.is_auto_named()) {
        truncate_end(&name, MAX_DERIVED_TAB_LABEL_WIDTH)
    } else {
        name
    };
...
fn max_tab_scroll(ws: &crate::workspace::Workspace, terminals: &Terminals, area: Rect) -> usize {
    (0..ws.tabs.len())
        .find(|&scroll| {
...
mod tests {
    ...
    #[test]
    fn active_tab_fg_contrasts_with_the_accent_luminance() {
        fn active_tab_foreground(accent: ratatui::style::Color) -> Option<ratatui::style::Color> {
            let mut app = AppState::test_new();
            ...
            terminal
                ...
                .unwrap();
            let tab_rect = app.view.tab_hit_areas[0];
            terminal.backend().buffer()[(tab_rect.x + 1, tab_rect.y)]
                .style()
                .fg
        }

        assert_eq!(
            active_tab_foreground(ratatui::style::Color::Rgb(230, 230, 230)),
            Some(ratatui::style::Color::Black)
        );
        assert_eq!(
            active_tab_foreground(ratatui::style::Color::Rgb(20, 25, 40)),
            Some(ratatui::style::Color::White)
        );
    }

    #[test]
    fn tab_bar_shows_the_derived_label_of_a_single_agent_tab() {
        let mut app = AppState::test_new();
        let ws = Workspace::test_new("test");
        ...
        let row = buffer_row_text(terminal.backend().buffer(), app.view.tab_bar_rect, 0);
        assert!(row.contains("fix the auth bug"), "tab row: {row:?}");
    }
