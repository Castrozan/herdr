...
impl AppState {
    ...
    fn dispatch_copy_mode_command(
        ...
        match command {
            ...
            CopyModeCommand::ScrollUp => self.scroll_copy_mode_by_lines(terminal_runtimes, -1, 1),
            CopyModeCommand::ScrollDown => self.scroll_copy_mode_by_lines(terminal_runtimes, 1, 1),
    ...
    fn scroll_copy_mode_page(
        ...
        self.scroll_copy_mode_by_lines(terminal_runtimes, direction, lines);
    }

    fn scroll_copy_mode_by_lines(
        &mut self,
        terminal_runtimes: &TerminalRuntimeRegistry,
        direction: i16,
        lines: usize,
...
mod tests {
    ...
    #[tokio::test]
    async fn copy_mode_scroll_commands_move_viewport_not_cursor() {
        let bytes = numbered_lines_bytes(64);
        let (mut app, pane_id) = app_with_copy_scrollback(&bytes);
        app.state.enter_copy_mode(&app.terminal_runtimes);
        ...
        for _ in 0..5 {
            app.state
                .dispatch_copy_mode_command(&app.terminal_runtimes, CopyModeCommand::ScrollUp);
        }

        assert_eq!(
            copy_mode_offset_from_bottom(&app, pane_id),
            5,
            "scroll-up moves the viewport 5 lines into history"
        );
        assert_eq!(
            app.state.copy_mode.as_ref().expect("copy mode").cursor_row,
            cursor_before,
            "scroll-up moves the viewport, not the cursor"
        );

        for _ in 0..5 {
            app.state
                .dispatch_copy_mode_command(&app.terminal_runtimes, CopyModeCommand::ScrollDown);
        }

        assert_eq!(
            copy_mode_offset_from_bottom(&app, pane_id),
            0,
            "scroll-down moves the viewport back toward the present"
        );
    }
